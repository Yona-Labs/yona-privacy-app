...
/**
 * Create a new ALT (always creates a new one, doesn't cache)
 */
export async function createNewALT(
  ...
  try {
    ...
    console.log(`Created new ALT: ${lookupTableAddress.toBase58()}`);
    console.log(`  Added ${addresses.length} addresses`);
...
export async function createGlobalTestALT(
  ...
  globalTestALT = await createNewALT(connection, payer, addresses);
  return globalTestALT;
}
 ...
 * Create a versioned transaction using an existing ALT
 */
export async function createVersionedTransactionBankrun(
  ...
  try {
    ...
    const mockLookupTable = {
      key: PublicKey.default, // Dummy key
      state: {
    ...
    const messageV0 = new anchor.web3.TransactionMessage({
      payerKey: payer,
      recentBlockhash: recentBlockhash[0],   
      instructions: instructions,
...
//  * Send and confirm a versioned transaction
//  */
// export async function sendAndConfirmVersionedTransactionBankrun(
//   banksClient: BanksClient,
//   transaction: VersionedTransaction,
//   signers: Keypair[]
// ): Promise<string> {
...
//   // Send and confirm the transaction
//   const signature = await banksClient.sendTransaction(transaction, {
//     skipPreflight: false,
//     preflightCommitment: 'confirmed',
//   });
